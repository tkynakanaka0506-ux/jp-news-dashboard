...
FEEDS = [
    ...
    ("米国株 ダウ ナスダック", "us", 1),
    # グローバル観測網(ユーザー要望2026-09-20): 「日本→世界の主要国政府・
    # 研究機関→企業→報道」という上流ほど早いはずの経路を、Google Newsの
    # 日本語検索クエリという既存の仕組みのまま広げる。海外機関の英語RSSを
    # 直接購読する案も検討したが、rules.jsonのキーワード判定が日本語前提
    # (英語見出しは一切マッチしない)なので、翻訳の仕組み無しに追加しても
    # 収集はできても分類できない画餅になる。翻訳(LLM任意)を前提にしない
    # 今の設計にまず合わせ、海外一次情報を報じた日本語記事を検索する形に
    # 留める(将来LLM翻訳を挟む拡張は別途検討)。
    ("米商務省 輸出規制", "trade", 2),
    ("米エネルギー省 補助金 半導体", "resources", 1),
    ("米国防総省 調達 契約", "geopolitics", 1),
    ("欧州委員会 規制 産業政策", "trade", 1),
    ("英国政府 産業戦略 投資", "trade", 1),
    ("中国政府 産業政策 輸出管理", "geopolitics", 1),
    ("韓国政府 半導体 支援策", "tech", 1),
    ("大学 研究開発 実用化 技術", "tech", 1),
]
